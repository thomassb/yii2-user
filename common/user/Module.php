...
class Module extends \yii\base\Module
{
    ...
    /**
     * @var array Model classes, e.g., ["User" => "common\user\models\User"]
     * Usage:
     *   $user = Yii::$app->getModule("user")->model("User", $config);
     *   (equivalent to)
     *   $user = new \common\user\models\User($config);
     *
    ...
    protected function checkModuleProperties()
    {
        ...
        if (!Yii::$app->request->isConsoleRequest && !Yii::$app->get("user") instanceof \common\user\components\User) {
            throw new InvalidConfigException('Yii::$app->user is not set properly. It needs to extend \common\user\components\User');
        }
    }
    ...
    {
        return [
            'User'       => 'common\user\models\User',
            'Profile'    => 'common\user\models\Profile',
            'Role'       => 'common\user\models\Role',
            'UserKey'    => 'common\user\models\UserKey',
            'UserAuth'   => 'common\user\models\UserAuth',
            'ForgotForm' => 'common\user\models\forms\ForgotForm',
            'LoginForm'  => 'common\user\models\forms\LoginForm',
            'ResendForm' => 'common\user\models\forms\ResendForm',
            'UserSearch' => 'common\user\models\search\UserSearch',
        ];
    }
