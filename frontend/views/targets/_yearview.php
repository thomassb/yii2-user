...
$levels=  \frontend\models\Levels::find()->all();
...
//    $form->field($model, 'created')->textInput();
    ?>
    <?= yii\bootstrap\Html::button('View All Years') ?>
    ...
    foreach($model as $target){
     //   echo ;
        //current year only
     $year= str_replace(' ', '',   strtolower($yeargroup->YearGroup));
    echo' <tr>
            <td>'.$listOfStrands[$target->StrandID].'</td>
            <td>'.$form->field($target, $year.'Target')->dropDownList($selectLevels,['class'=>'form-control year input-sm', 'prompt'=>''])->label(false).'</td>
                <td>'.$form->field($target, $year.'ReviewedTarget')->textInput(['class'=>'form-control year input-sm'])->label(false).'</td>
        </tr>';
